...
## ADR-019 — `ConsultarPedido` deixa de ser obrigatório no fluxo normal de Pedidos

- Status: aceito — 2026-08-21
- Decisão: a sincronização base de `sales_orders` (`ListarPedidos`) passa a extrair diretamente `infoCadastro.cancelado`/`dCan`/`hCan`/`dFat` e `lista_parcelas.parcela`, persistindo `sales_order_installments` e derivando `real_due_date`/`invoice_date`/`is_cancelled`/`cancelled_at` sem chamada adicional. `ConsultarPedido` (e sua fila de enriquecimento) não foi removido — continua disponível para diagnóstico pontual, reconciliação ou casos em que um detalhe futuro não venha na listagem — mas deixa de ser parte obrigatória do fluxo normal.
- Motivo: evidência real (Onda 3, 435 pedidos, 56 OS) mostrou que `infoCadastro` e `lista_parcelas` já vêm 100% preenchidos em `ListarPedidos`/`ListarOS`; um lote controlado de `ConsultarPedido` inicialmente reproduziu os mesmos dados por trazerem exatamente a mesma informação (após corrigir um bug real de desembrulhamento de `pedido_venda_produto`). Eliminar a obrigatoriedade evita ~420 chamadas individuais desnecessárias (a ~4,8s cada) sem perda de informação conhecida.
- Mesma lógica aplicada a `service_orders`: `InfoCadastro.cCancelada` (cancelamento) e `dDtFat`/`cFaturada` (faturamento) já vinham da listagem desde a implementação original; `analytics.sales` foi corrigida para de fato excluir OS cancelada dos indicadores (antes sempre mostrava `null`).
- Vínculo Pedido/OS ↔ títulos financeiros: investigado com dados reais, nenhuma chave explícita e confiável encontrada (ver `13-OPEN-QUESTIONS.md`). Domínios permanecem separados — nenhuma heurística foi implementada.

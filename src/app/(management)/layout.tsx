...
export default async function ManagementLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!auth.profile?.is_active) return <main className="auth-state"><section className="login-panel"><div className="brand-placeholder">CC</div><p className="eyebrow">ACESSO BLOQUEADO</p><h1>Perfil inativo</h1><p>Solicite a ativação do seu perfil a um administrador.</p><form action={logout}><button className="secondary-button">Sair</button></form></section></main>;

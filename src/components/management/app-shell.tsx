...
export function AppShell({ children, role, userName, logoutAction }: { children: React.ReactNode; role: UserRole; userName: string; logoutAction: () => Promise<void> }) {
  ...
  return <div className="app-frame">
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`} aria-label="Navegação principal">
      <div className="brand-lockup"><Image className="brand-mark" src="/branding/logo-chapa-certa-fundo-escuro.png" alt="Chapa Certa" width={54} height={38} priority /><div><strong>CHAPA CERTA</strong><span>Central de Gestão</span></div></div>
    ...
    <div className="workspace"><header className="topbar"><button className="mobile-menu" onClick={() => setOpen(true)} aria-label="Abrir navegação">☰</button><div className="topbar-brand"><Image src="/branding/logo-chapa-certa-fundo-claro.png" alt="Chapa Certa" width={23} height={24} /></div><div className="topbar-status"><span className="environment-dot" /> Dados gerenciais protegidos</div><div className="topbar-user">{userName}<span>{role}</span></div></header><main className="management-main">{children}</main></div>
  </div>;
}
